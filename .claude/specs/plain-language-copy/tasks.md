...
**Acceptance:**
- [ ] `ko`, `en`만 지원하고 기본 대체 언어는 `en`이다.
- [ ] 영어 리소스에서 키가 빠지거나 구조가 달라지면 TypeScript 검사가 실패한다.
- [ ] 기존 화면은 이 단계에서도 빌드된다.
...
**Acceptance:**
- [ ] 유효한 `trendar.uiLocale`이 있으면 위치 API를 호출하지 않는다.
- [ ] API 실패·잘못된 저장값에서도 앱이 렌더되고 `html[lang]`이 선택 언어와 일치한다.
- [ ] 언어 판정 중 번역 전 본문이 깜빡이지 않는다.
...
**Acceptance:**
- [ ] `KR`, 비한국 국가, 국가 없음, 다중 언어, 잘못된 `q` 값의 결과가 승인된 우선순위와 일치한다.
- [ ] 헤더는 512자·10개 항목까지만 처리하고 항상 지원 언어 하나를 반환한다.
...
**Acceptance:**
- [ ] 사설·루프백·잘못된 IP는 외부로 전송하지 않는다.
- [ ] 정상 응답에서는 ISO 국가 코드만 반환하고 모든 외부 오류는 `null`로 정규화한다.
- [ ] 원본 IP·비밀값을 저장하거나 로그에 남기지 않고 캐시는 10분·5,000개로 제한된다.
...
**Acceptance:**
- [ ] `GET /api/localization`은 인증 없이 `{ ok: true, data: { locale } }`만 반환한다.
- [ ] 외부 조회가 실패해도 `200`과 브라우저 언어 또는 영어를 반환한다.
- [ ] 응답은 `private, no-store`이며 IP·국가·공급자·실패 이유를 포함하지 않는다.
...
- [ ] MaxMind 키가 없는 로컬 환경에서도 브라우저 언어로 앱이 열린다.
...
**Acceptance:**
- [ ] 한국어와 영어에서 숫자·날짜·상대 시간이 각 locale 규칙에 맞는다.
- [ ] null·잘못된 날짜 처리와 기존 축약 형식이 회귀하지 않는다.
...
**Acceptance:**
- [ ] 언어 변경 시 열린 공통 상태와 대화상자 문구가 즉시 바뀐다.
- [ ] 버튼의 접근 가능한 이름과 기존 키보드 동작을 유지한다.
- [ ] 내부 오류 세부사항을 공통 오류 상태에서 표시하지 않는다.
...
**Acceptance:**
- [ ] `한국어`, `English`를 키보드로 선택할 수 있고 현재 언어가 접근 가능하게 표시된다.
- [ ] 선택 즉시 화면과 `html[lang]`이 바뀌고 새로고침 후 유지된다.
- [ ] `Dashboard`, `Queries`, `Repos`, `mock`이 쉬운 현재 언어 문구로 교체된다.
...
**Acceptance:**
- [ ] 전문 용어를 승인된 쉬운 용어로 교체하고 모든 로그인 화면 문구가 두 언어에 존재한다.
- [ ] 언어 선택기와 국가 단위 위치 일시 확인 안내가 로그인 전에도 보인다.
- [ ] GitHub 로그인 동작과 OAuth 오류 분기는 유지된다.
...
- [ ] 저장 언어 없음·한국/비한국·위치 실패·수동 선택의 우선순위가 맞다.
- [ ] 로그인 화면과 상단 메뉴가 320px·데스크톱에서 두 언어로 동작한다.
...
**Acceptance:**
- [ ] `Repos`, `Active Queries`, `Last ETL` 같은 표현이 승인된 쉬운 용어로 바뀐다.
- [ ] 숫자와 마지막 수집 시간이 현재 언어 형식으로 표시된다.
...
**Acceptance:**
- [ ] `Repo`, `Lang`, `Δ24H`, `TOP MOVERS`, `RISING`, `BOOKMARKS`가 쉬운 현재 언어 문구로 바뀐다.
- [ ] 툴팁과 접근 가능한 이름에도 기술 약어가 남지 않는다.
...
**Acceptance:**
- [ ] 사용자 화면에서 `ETL`, `Query`, `Repos`, `Actions`, `KST`가 승인된 표현으로 바뀐다.
- [ ] 키워드·주제 검색의 차이와 수집 결과 단위가 비전문가도 이해할 문장으로 표시된다.
- [ ] CRUD·개별/전체 수집·한도 동작은 유지된다.
...
**Acceptance:**
- [ ] `repo/레포`, `Lang`, `Stars`, `Δ`, `Actions`가 승인된 쉬운 표현으로 바뀐다.
- [ ] 검색·정렬·필터·북마크·삭제·페이지 이동 기능과 숫자 형식이 유지된다.
...
**Acceptance:**
- [ ] `query`, `snapshot`, `issues`, `delta`가 승인된 쉬운 표현으로 바뀐다.
- [ ] 날짜·숫자·차트 툴팁이 현재 언어 형식으로 표시된다.
- [ ] 메모·북마크·삭제와 외부 GitHub 링크 동작은 유지된다.
...
**Acceptance:**
- [ ] `query_id`, `query_type`, HTTP 번호, 서버 내부 메시지가 토스트·오류 상태에 직접 노출되지 않는다.
- [ ] 네트워크·인증·검증·중복·한도·찾을 수 없음 오류가 현재 언어로 표시된다.
- [ ] 예시 데이터와 실제 API의 같은 오류 코드가 같은 문구를 만든다.
...
**Acceptance:**
- [ ] 오류 응답과 수집 결과에 `query_id`, `query_type`, `base/trend`, 내부 ID, 원시 외부 오류가 노출되지 않는다.
- [ ] `ETL`, `쿼리`, `저장소`, `KST` 사용자 표현이 `수집`, `수집 조건`, `프로젝트`, `한국 시간`으로 바뀐다.
- [ ] 서버 로그는 진단 정보를 유지하되 응답·DB 상태 메시지와 분리된다.
...
- [ ] `cd backend && npm test`
- [ ] `cd frontend && npm run typecheck && npm run build`
- [ ] 한국어와 영어의 주요 사용자 흐름에서 하드코딩된 전문 용어와 번역 키 누락이 없다.
...
**Acceptance:**
- [ ] 로그인·상단 메뉴·수집 조건·프로젝트 목록·상세에서 가로 넘침과 잘린 주요 동작이 없다.
- [ ] 언어 선택과 대화상자를 키보드로 사용할 수 있고 포커스가 보인다.
- [ ] 보정이 5개 파일을 넘으면 화면별 후속 태스크로 나눠 진행한다.
...
- [ ] 사용자 화면에서 금지 용어와 내부 구현 정보가 남아 있지 않다.
- [ ] localization 응답·로그·빌드 산출물·diff에 IP, 국가 코드, MaxMind 키가 없다.
- [ ] 요구사항 R1~R8을 증거와 함께 셀프 리뷰한다.
